...
export const CarsApi = {
  list: () => adminRequest<Car[]>({ url: "/cars/" }),
  create: (data: CarPayload) => adminRequest<Car>({ url: "/cars/", method: "POST", data }),
  update: (id: number, data: CarPayload) => adminRequest<Car>({ url: `/cars/${id}`, method: "PUT", data }),
  ...
  async upload(file: File) {
    ...
    const result = await adminRequest<{ filename: string }>({ url: "/cars/upload-image", method: "POST", data })
