...
export function BrandsPage() {
  ...
  const [editing, setEditing] = useState<EditingBrand>(() =>
  ...
  const saveMutation = useMutation({
    mutationFn: ({
      ...
      brand
        ? BrandsApi.update(brand.id, payload)
        : BrandsApi.create(payload),
    onSuccess: async (_, { brand, payload }) => {
      toast.success(
        t(
          brand
            ? "admin.feedback.brands.updated"
            : "admin.feedback.brands.created",
        ),
        t("admin.feedback.brands.saved", {
          name:
            language === "ar" ? payload.name_ar : payload.name_en,
        }),
      )
  ...
  const filtered = useMemo(() => {
    ...
    return (query.data ?? []).filter(
      (brand) =>
        !term ||
        brand.name_en.toLocaleLowerCase().includes(term) ||
        brand.name_ar.toLocaleLowerCase().includes(term),
    )
  }, [query.data, search])
  ...
  async function deleteBrand(brand: Brand) {
    ...
    if (
      !(await confirm({
        title: t("admin.feedback.brands.deleteTitle"),
        message: t("admin.feedback.brands.deleteMessage", { name }),
        confirmLabel: t("admin.feedback.brands.deleteConfirm"),
        variant: "danger",
      }))
    ) {
      return
    ...
    try {
      ...
      toast.success(
        t("admin.feedback.brands.deleted"),
        t("admin.feedback.brands.removed", { name }),
      )
  ...
  return (
    <section>
      <PageHeader
        eyebrow={t("admin.pages.brandsEyebrow")}
        icon="brands"
        title={t("admin.brands")}
        description={t("admin.pages.brandsDescription")}
        actions={
          <button
            className="button"
            onClick={() => setEditing(null)}
          >
            + {t("admin.pages.addBrand")}
          </button>
        }
      />
      ...
      <LoadableContent
        ...
        loadingMessage={t("admin.management.loadingBrands")}
        error={
          query.error
            ? getLocalizedErrorMessage(
                query.error,
                language,
                t("admin.management.loadBrandsError"),
              )
            : ""
        }
      ...
      >
        {items.length === 0 ? (
          <EmptyState
            title={t("admin.management.noBrands")}
            message={t("admin.management.noBrandsMessage")}
            action={
              <button
                className="button"
                onClick={() => setEditing(null)}
          ...
          <div className="table-card">
            <div className="product-table product-table--head brand-table">
              <span>{t("admin.management.brand")}</span>
              <span>{t("admin.management.created")}</span>
              <span>{t("admin.management.actions")}</span>
            </div>

            {items.map((brand) => (
              <article
                className="product-table brand-table"
                key={brand.id}
              >
...
function BrandFormDialog({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof BrandPayload, string>>
  >({})
  ...
  async function submit(event: FormEvent) {
    ...
    if (!form.name_en.trim()) {
      nextErrors.name_en = t("admin.validation.requiredField")
    }

    if (!form.name_ar.trim()) {
      nextErrors.name_ar = t("admin.validation.requiredField")
    }
    ...
    <div className="dialog-backdrop">
      ...
      >
        <div className="form-dialog__header">
          <div>
            <span>{t("admin.forms.brand.eyebrow")}</span>
            <h2>
              {t(
                brand
                  ? "admin.forms.brand.editTitle"
                  : "admin.forms.brand.newTitle",
              )}
            </h2>
          </div>
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setErrors({ ...errors, name_en: undefined })
              }}
              required
            />
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setErrors({ ...errors, name_ar: undefined })
              }}
              required
            />
